...
test('las empresas de práctica no se cuelan en el feed', () => {
  const src = feedFuente()
  /**
   * Se exige el AYUDANTE del repo, no la condición escrita a mano.
   *
   * Esta prueba comprobaba el texto literal `company: { esDemo: false }`, que
   * es el valor de `SIN_DEMO` copiado. Vigilar la ortografía tiene dos
   * precios: rechaza el código que usa el ayudante —que es el correcto— y no
   * se entera el día que `SIN_DEMO` cambie y esta copia se quede atrás. Un
   * `where` de demo que no sigue al de todos los demás es justo el que se
   * olvida.
   */
  assert.match(src, /company: \{ \.\.\.SIN_DEMO \}/)
  assert.match(fuente(...QUERIES), /import \{ SIN_DEMO \} from '@\/modules\/demo'/)
