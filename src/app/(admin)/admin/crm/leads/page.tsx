  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Gestiona tus leads en el pipeline de ventas manual.
        </p>
      </div>

      <Form action="/admin/crm/leads" className="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      {paginas > 1 && (
        <nav className="flex items-center justify-between gap-3" aria-label="Paginación">
          <PaginationLink
            href={buildUrl('/admin/crm/leads', sp, { p: result.pagina - 1 > 1 ? String(result.pagina - 1) : undefined })}
          ...
          <PaginationLink
            href={buildUrl('/admin/crm/leads', sp, { p: String(result.pagina + 1) })}
