...
/** El sobre de quien está mirando. Nunca lanza: sin sesión, de fábrica. */
export async function misPreferenciasReportes(): Promise<PreferenciasReportes> {
  const user = await getUser()
  const id = user?.metadata?.dbUserId
  if (!id) return PREFERENCIAS_VACIAS
  const fila = await sinEmpresa('reportes: preferencias del usuario', (tx) =>
    tx.user
      .findUnique({ where: { id }, select: { preferenciasReportes: true } })
      .catch(() => null)
  )
  return leerPreferencias(fila?.preferenciasReportes)
}
...
async function guardar(
  ...
  const actual = await misPreferenciasReportes()
