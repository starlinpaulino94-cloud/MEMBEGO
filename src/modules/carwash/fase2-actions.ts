...
export async function guardarCuenta(_prev: Fase2State, formData: FormData): Promise<Fase2State> {
  try {
    ...
    if (id) {
      const upd = await prisma.cuentaCorporativa.updateMany({
        where: { id, companyId: ctx.companyId },
        data: datos,
      })
      if (upd.count === 0) return { error: 'Cuenta no encontrada.' }
    } else {
      const existe = await prisma.cuentaCorporativa.findFirst({
      ...
      await prisma.cuentaCorporativa.create({ data: { companyId: ctx.companyId, ...datos } })
    }
...
export async function alternarCuenta(id: string): Promise<Fase2State> {
  try {
    ...
    await prisma.cuentaCorporativa.update({ where: { id }, data: { activa: !c.activa } })
    revalidatePath(RUTA_CUENTAS)
...
export async function agregarVehiculoCuenta(
  ...
  try {
    ...
    const cuenta = await prisma.cuentaCorporativa.findFirst({
    ...
    const enOtra = await prisma.cuentaVehiculo.findFirst({
    ...
    await prisma.cuentaVehiculo.upsert({
...
export async function quitarVehiculoCuenta(id: string): Promise<Fase2State> {
  try {
    const ctx = await contexto('CUENTAS_CORPORATIVAS')
    if ('error' in ctx) return ctx
    const v = await prisma.cuentaVehiculo.findFirst({
    ...
    await prisma.cuentaVehiculo.update({ where: { id }, data: { activo: false } })
...
export async function cerrarCorte(_prev: Fase2State, formData: FormData): Promise<Fase2State> {
  try {
    ...
    const upd = await prisma.cargoCuenta.updateMany({
...
export async function pagarComisiones(
  ...
  try {
    ...
    const upd = await prisma.comision.updateMany({
...
export async function resolverIncidencia(
  ...
  try {
    ...
    const upd = await prisma.incidencia.updateMany({
      where: { id, companyId: ctx.companyId },
      data: {
        estado,
...
export async function ordenarRewash(incidenciaId: string): Promise<Fase2State> {
  try {
    ...
    const nueva = await prisma.colaVehiculo.create({
      data: {
        companyId: ctx.companyId,
