...
export async function getNovedadesInicio(
  ...
  try {
    ...
    /**
     * Las empresas de práctica quedan fuera.
     *
     * Datos de demostración no se mezclan con producción: una empresa de
     * práctica que alguien siguió probando metería ofertas que no existen en
     * el mismo feed que las de verdad, y el cliente no tendría forma de
     * distinguirlas.
     *
     * ESTO ROZA EL BORDE de la regla escrita en `SIN_DEMO`, y queda anotado a
     * propósito. Aquella dice que la exclusión va en lo que DESCUBRE empresas
     * —donde no las pidió quien mira— y NO en lo que la persona ya tiene,
     * porque si no «el entrenamiento enseñaría pantallas vacías». Seguir a una
     * empresa es pedirla, así que este feed cae justo en medio.
     *
     * Se resuelve hacia excluir porque el coste de equivocarse no es simétrico:
     * una pantalla de novedades sin la empresa de práctica es un hueco durante
     * el entrenamiento; una oferta de práctica mezclada con las reales es un
     * cliente que va al negocio a reclamar algo que no existe.
     */
    const empresaReal = { companyId: { in: companyIds }, company: { ...SIN_DEMO } } as const
